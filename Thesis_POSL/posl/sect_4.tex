...
%With the operator $(\posldot)$ we can have access to \oms{} sending information and to the \opch's names in a solver. 
%For example: $Solver_0\posldot \mathcal{M}$ provides access to the \om{} $\mathcal{M}$ in $Solver_0$ if and only if it is affected by a {\it sending} operator, and $Solver_1\posldot CM$ provides access to the \opch{} $CM$ in $Solver_1$.
...
\begin{enumerate} 
\item $\mathcal{J} = \left[\mathcal{S}_0\posldot \mathcal{M}_0, \mathcal{S}_1\posldot \mathcal{M}_1,\dots, \mathcal{S}_{N-1}\posldot \mathcal{M}_{N-1}\right]$ be the list of \jacks, and 
\item $\mathcal{O} = \left[\mathcal{S}_0\posldot \mathcal{CM}_0, \mathcal{S}_1\posldot \mathcal{CM}_1,\dots, \mathcal{S}_{N-1}\posldot \mathcal{CM}_{N-1}\right]$ be the list of \outlets{} 
\end{enumerate} Then the operation 
\[
\mathcal{J} \ringsep \mathcal{O}
\]
connects each \jack{} $\mathcal{S}_i\posldot \mathcal{M}_i \in \mathcal{J}$ with the corresponding \outlet{} $\mathcal{Z}_{(i+1)\%N}\posldot \mathcal{CM}_{(i+1)\%N} \in \mathcal{O}$, $\forall 0 \leq i \leq N-1$  (see Figure~\ref{subfig:comm_ring}).
\end{definition}
...
\poslexample{These operators can be combined between themselves to contruct any kind of \comstr. Figure~\ref{fig:ex:comb} shows a simple example combining solvers doubly connected and non connected solvers. Assuming that all solvers $S_i, i\in[1..5]$ have a module $\mathcal{M}$ sent by a send operator, and a \opch{} $\mathcal{CM}$, the corresponding code is the following:
\begin{gather*}
\left[\mathcal{S}_1\posldot\mathcal{M}, \mathcal{S}_2\posldot\mathcal{M}, \mathcal{S}_{3}\posldot\mathcal{M}\right] \onetonsep \left[\mathcal{S}_4\posldot\mathcal{CM}, \mathcal{S}_5\posldot\mathcal{CM}\right]\\
\left[\mathcal{S}_4\posldot\mathcal{M}, \mathcal{S}_5\posldot\mathcal{M}\right] \onetoonesep \left[\mathcal{S}_1\posldot\mathcal{CM}, \mathcal{S}_{3}\posldot\mathcal{CM}\right]\\
...
$$\left[S\posldot A\right] \onetonsep \left[Z\posldot C.M.\right]$$
If the operator {\bf 1~to~N} is used with only one solver in each list, the operation is equivalent to applying the operator {\bf 1~to~1}. However, to obtain a communication strategy like the one showed in Figure~\ref{subfig:comm_diff}, six solvers (three senders and three receivers) have to be declared to be able to apply the following operation:
$$\left[S_1\posldot A, S_2\posldot A, S_3\posldot A\right]\onetonsep \left[Z_1\posldot C.M., Z_2\posldot C.M., Z_3\posldot C.M.\right]$$
\posl{} provides a mechanism to make this easier, through two {\it syntactic sugars}, called formally \textit{solver namespace expansions}, explained in the next sub-section. %below.
}
...
\new{The two types of namespace expantion provided by \posl{} are the following:}
\begin{enumerate}
...
\textbf{Solver name expansion - } Uses an integer $K$ to denote how many times the solver name $S$ will appear in the declaration. $\left[\dots S_i\posldot\mathcal{M}(K),\dots\right]$ expands as $\left[\dots S_i\posldot\mathcal{M}, S_i^2\posldot\mathcal{M},\dots S_i^K\posldot\mathcal{M}\dots\right]$\\
...
\poslexample{Suppose that we have created  solvers $S$ and $Z$ mentioned in the previews example. As a communication strategy, we want to connect them through the operator {\bf 1~to~N}, using $S$ as sender and $Z$ as receiver. Then,  %using \textbf{namespace expansions}, 
we need to declare how many solvers we want to connect. Algorithm~\ref{algo:comm_str_ex} shows the desired communication strategy. Notice in this example that the connection operation is affected also by the number $2$ at the end of the line. %, as {\bf connection declaration expansion}. 
...
\SetNoline
%\myproc{
[ S$\posldot A$ (3) ] $\onetonsep$ [ Z$\posldot C.M.$ (3) ] 2 ;
