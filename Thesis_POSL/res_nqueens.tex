...
Solvers used for the experiments without communications, are presented in Algorithm~\ref{as:nq}, where the \as{} is instantiated in the solver \solverposl{as} with the neighborhood \om{} $V_{AS}$, which given a configuration, returns a neighborhood $V\left(s\right)$ swapping the variable which contributes the most to the cost, with all others. This solver was able to find solutions but taking too much time (a minute for 6000-queens, for example). For that reason it was implemented a neighborhood \om{} $S_{PAS}(p)$ which performs the same algorithm of $S_{AS}$, but instead of generating neighbors swapping the most costly variable with all others, it is swapped only with a percentage of rest of variables. Solver \solverposl{selective} instantiates the \as{} with this \om{} and it showing much better results than \solverposl{as}.

Table~\ref{tab:nqueens_seqpar} shows that the improvement of the parallel scheme using \posl{} is not big. \tet{Si bien la cantidad de soluciones de este problema es solo conocida hasta un numero muy pequenno de N (23), estudios sugieren que la cantidad de soluciones crece exponencialmente con N. Esto implica que a medida que el problema crece en orden, es más fácil arrivar a una solucion utilizando local search methods. El comportamiento de \posl{} matches justamente con esta hipotesis: el proceso de busqueda en instancias grandes es más estable y la convergencia es más directa. En un espacio de busqueda bien repartido, el paralelismo no aporta gran cosa, a no ser que se utilicen muchos más cores (propuesto para estudio futuro).}
...
\begin{tabular}{p{1.3cm}|R{1.3cm}R{1.3cm}R{1.3cm}R{1.3cm}|R{1.3cm}R{1.3cm}R{1.3cm}R{1.3cm}}
	...
	6000 & 4.709 & 0.183 & 3,339 & 51 & 4.57 & 0.123 & 3,323 & 43 \\
	\hline
\end{tabular}
}
