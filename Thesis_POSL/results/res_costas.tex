...
\new{The main idea of the proposed \commstr{} (variant A) follows the following steps:}

\poslexample{
\mybox{Step 1}

\new{The \om{} $I_{perm}$ generates an initial configuration $s$ respecting the structure of the problem's model, \ie the configuration $s$ is a permutation of the vector $[1..N]$. The configuration $s$ is the input of the \om{} in the Step 2.}

\mybox{Step 2}

\new{The \om{} $R_{AS}$ takes a configuration $s$ as input and returns a configuration $\hat{s}$ as result of performing a reset.}

\mybox{Step 3}

\new{The \om{} $V_{AS}$ takes a configuration $\hat{s}$ as input and returns the neighborhood $\mathcal{V}\left(\hat{s}\right)$ generated by swapping the most culprit variable with all other variables.}

\mybox{Step 4}

\new{The \om{} $S_{first}$ selects the first configuration $s' \in V\left(\hat{s}\right)$ improving the current cost, and returns it together with the current configuration, into the pair $\left(s', \hat{s}\right)$.}

\mybox{Step 5}

\new{\underline{Sender solver}: A send data operator is executed, which takes the input $\left(s', \hat{s}\right)$ from the Step 4 and executes the module $A_{AI}$ (which returns always the selected configuration $s'$) and sends its output to external (receivers) solvers.}

\new{\underline{Receiver solver}: A \cm{} which combines the \om{} $A_{AI}$ and the \opch{} $CM_{last}$ trough the operator $\poslop{m}$ is executed. The minimum operator takes the input of the Step 4 and executes $A_{AI}$ which returns its output $s'$, and executes $CM_{last}$, which returns the received configuration $s''$. The \cm{} returns the configuration with the lowest cost between $s'$ and $s''$.}

\new{In both cases, the process returns either to the Step 3 if the current iteration is not congruent with 0 modulo $K_2$, \ie if ${\Iter \% K_2 = 0}$; or to the Step 2, otherwise.}
}
...
\new{The random nature of this solution strategy has showed to be effective. However, it is the explanation to the high values of standard deviation showed in Table~\ref{tab:costas19comm}. \Ass{} used for the resolution of \CARRP{} combine \oms{} which take many stochastic decisions. The neighborhood \om{} compute the neighborhood based on the most culprit variable of the input configuration, which is randomly selected if there exist more than one. This \m{} also generates neighbors by selecting randomly the variables to swap. In addiction, the reset \om{} generates configurations in three different ways and equally probable, for a same input configuration.}
