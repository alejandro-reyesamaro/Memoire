...
\modified{As we said before, the \as{} is independent from the \oms{} and \opchs{} used in the solver, it means that one \as{} can be used to construct many different solvers, by implement it using different modules (see below the concept of \as{} instantiation). For that reason, the \as{} is defined only using {abstract} modules. Formally, we define an \as{} as follows:}
...
An \as{} is defined as follow: after declaring an \mbox{\tet{\bf abstract solver}} and its name, the first line defines the list of abstract \oms, the second one the list of abstract \opchs, then the algorithm of the solver is defined is the solver's body (the root \cm), between \mbox{\tet{\bf begin}} and \mbox{\tet{\bf end}}.

\modified{So, formally, we can present the declaration of an \as{} in the following simple regular expression:}
...
For instance, Algorithm~\ref{algo:as_example} illustrate the abstract solver corresponding to Figure~\ref{subfig:as}.
